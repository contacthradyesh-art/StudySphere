...
/**
 * Generate a plain-text answer, optionally grounded with Google Search
 * for up-to-date, research-backed responses (current affairs, recent facts, etc).
 * Falls back gracefully to the model's own knowledge if search grounding
 * isn't available for the given API key/model.
 */
export async function generateTextAnswer(options: {
  prompt: string;
  systemInstruction?: string;
  useSearch?: boolean;
    ...
    try {
      ...
      const result = await model.generateContent({
        ...
        systemInstruction: options.systemInstruction
          ? { role: "system", parts: [{ text: options.systemInstruction }] }
          : undefined,
        // Google Search grounding — gives the model live web results to cite.
        tools: [{ googleSearchRetrieval: {} } as any],
      });
      return { text: result.response.text(), usedSearch: true };
    } catch (error) {
      console.warn("Search-grounded generation failed, falling back to plain answer:", error);
      // fall through to non-search generation below
    }
  }
  ...
  return { text: result.response.text(), usedSearch: false };
}
