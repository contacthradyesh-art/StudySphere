...
import { MAX_UPLOAD_SIZE_MB, MAX_UPLOAD_SIZE_BYTES, type LibraryFile, type LibraryFolder } from '@/lib/mission-ias/library-schema';
...
// don't exist in Node.js. Next.js prerenders 'use client' pages on the
// server too, which was crashing the build. Loading it client-only fixes
// that without losing anything — the reader only ever renders after a user
// clicks "Open" anyway, so there's no meaningful loading-state UX to add.
...
export default function DigitalLibraryPage() {
  ...
  async function uploadFiles(fileList: FileList | File[]) {
    ...
    const tooBig = files.filter((f) => f.size > MAX_UPLOAD_SIZE_BYTES);
    if (tooBig.length > 0) toast.error(`${tooBig.length} file(s) skipped \u2014 over the ${MAX_UPLOAD_SIZE_MB}MB limit`);

    const toUpload = files.filter((f) => f.size <= MAX_UPLOAD_SIZE_BYTES);
    if (toUpload.length === 0) return;

    setUploading(true);
    setUploadProgress(toUpload.map((f) => ({ name: f.name, percent: 0 })));
    let succeeded = 0;
    let failed = 0;
    ...
    await Promise.all(
      toUpload.map(async (file, i) => {
        const { promise } = uploadLibraryFile(
          user.uid,
          file,
          { subject: uploadSubject, folderId: activeFolder === 'all' ? null : activeFolder },
          (percent) => setUploadProgress((prev) => prev.map((p, idx) => (idx === i ? { ...p, percent } : p)))
        );
        try {
          await promise;
          succeeded++;
    ...
    <div className="space-y-5 animate-fade-in">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex gap-2">
          <input ref={fileInputRef} type="file" multiple className="hidden" onChange={handleFileChosen} accept=".pdf,.doc,.docx,.ppt,.pptx,.png,.jpg,.jpeg,.webp,.txt,.zip" />
              ...
              {uploadProgress.map((p) => (
                <div key={p.name} className="space-y-1">
                  ...
                  <div className="h-1.5 w-full overflow-hidden rounded-full bg-white/10">
                    <div className="h-full rounded-full bg-gradient-brand transition-all" style={{ width: `${p.percent}%` }} />
                  </div>
                </div>
              ))}
            </div>
                ...
                <GlassCard key={file.id} className={cn('space-y-2', view === 'list' && 'flex flex-row items-center gap-3 space-y-0')}>
                  ...
                  <div className="flex flex-wrap items-center gap-2 pt-1">
                    ...
                    >
                      ...
                      {file.type.includes('pdf') && file.lastPage ? `Continue — Page ${file.lastPage}` : 'Open'}
                    </button>
      ...
      {readerFile && <PdfReader file={readerFile} onClose={() => setReaderFile(null)} />}
    </div>
  );
}