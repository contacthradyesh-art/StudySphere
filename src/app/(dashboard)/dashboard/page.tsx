          ...
          <div className="space-y-2">
            {pendingToday.map((t) => (
              <button
                ...
                className="flex w-full items-center gap-3 rounded-xl border border-white/[0.06] bg-white/[0.03] px-4 py-3 text-left transition-colors hover:bg-white/[0.06]"
              >
        ...
        <div className="-mx-4 flex snap-x gap-3 overflow-x-auto px-4 pb-2 scrollbar-hide sm:mx-0 sm:px-0">
          {TOOLS.map((t) => (
            <Link
              key={t.title}
              href={t.link}
              className="group block w-40 shrink-0 snap-start rounded-2xl border border-white/[0.06] bg-white/[0.03] p-4 transition-colors hover:border-primary/30 hover:bg-white/[0.06]"
            >
        ...
        {badges.length > 0 && (
          <div className="mt-4 flex flex-wrap gap-2">
            {badges.slice(0, 6).map((b) => (
              <div key={b.id} title={b.description} className="flex items-center gap-1.5 rounded-full border border-white/[0.08] bg-white/5 px-3 py-1.5 text-xs">
