...
export function HabitDialog({ open, onClose, onSubmit, saving = false }: HabitDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4 backdrop-blur-sm" onClick={() => !saving && onClose()}>
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              {FREQUENCIES.map((f) => (
                <Button key={f.id} type="button" size="sm" variant={frequency === f.id ? 'gradient' : 'outline'} className="flex-1" onClick={() => setFrequency(f.id)} disabled={saving}>
                  {f.label}
                </Button>
              ))}
            </div>
          ...
          {frequency === 'custom' && (
            <div className="space-y-3 rounded-xl border border-input p-3">
              <div className="flex gap-2 text-xs">
                <button type="button" onClick={() => setCustomMode('weekdays')} disabled={saving} className={customMode === 'weekdays' ? 'font-semibold text-primary' : 'text-muted-foreground'}>Specific days</button>
                <span className="text-muted-foreground">·</span>
                <button type="button" onClick={() => setCustomMode('count')} disabled={saving} className={customMode === 'count' ? 'font-semibold text-primary' : 'text-muted-foreground'}>X times / week</button>
              </div>

              {customMode === 'weekdays' ? (
                <div className="flex gap-1.5">
                  {WEEKDAYS.map((label, i) => (
                    <button key={i} type="button" onClick={() => toggleDay(i)} disabled={saving} className={`h-8 w-8 rounded-full text-xs font-medium ${customDays.includes(i) ? 'bg-gradient-brand text-white' : 'bg-secondary text-muted-foreground'}`}>{label}</button>
                  ))}
                </div>
              ) : (
                <div className="flex items-center gap-2">
                  <Input type="number" min={1} max={7} value={timesPerPeriod} onChange={(e) => setTimesPerPeriod(Number(e.target.value))} className="w-20" disabled={saving} />
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              {ACCENTS.map((c) => (
                <button key={c} type="button" onClick={() => setColor(c)} disabled={saving} className="h-7 w-7 rounded-full border-2 transition-transform" style={{ background: c, borderColor: color === c ? '#fff' : 'transparent', transform: color === c ? 'scale(1.15)' : 'scale(1)' }} />
              ))}
            </div>
          </div>
