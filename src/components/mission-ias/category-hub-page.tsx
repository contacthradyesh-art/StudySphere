...
export function CategoryHubPage({
  title, description, icon: Icon, category, noteCategory
}: {
  title: string;
  description: string;
  ...
  async function handleSaveToNotes(item: CurrentAffairsItem) {
    ...
    try {
      await createNote(user.uid, {
        ...
        tags: [item.category, noteCategory.toLowerCase().replace(/\s+/g, '-')]
      });
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      {loading && <GlassCard><p className="text-sm text-muted-foreground">Loading...</p></GlassCard>}

      {!loading && filtered.length === 0 && (
        <GlassCard>
          <p className="text-sm text-muted-foreground">
            Nothing here yet \u2014 this fills up automatically as relevant news comes in via Current Affairs.
          </p>
        </GlassCard>
      )}
