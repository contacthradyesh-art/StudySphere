...
import { Bell, Check, LogOut, Moon, Search, Settings, Sun, User as UserIcon, X } from 'lucide-react';
    ...
    <header className="sticky top-0 z-40 flex h-16 min-w-0 items-center justify-between border-b border-border/60 bg-background/85 px-3 backdrop-blur-xl sm:px-4 md:px-6">
      ...
      <div className="ml-auto flex items-center gap-1">
        ...
        <Button
          ...
          aria-label={mounted ? `Switch to ${isDark ? 'light' : 'dark'} mode` : 'Theme'}
        ...
        <div className="relative" ref={notificationRef}>
          <Button variant="ghost" size="icon" className="relative h-10 w-10 rounded-xl hover:bg-muted" aria-label={`${pending.length} pending notifications`} aria-expanded={notificationsOpen} onClick={() => { setNotificationsOpen((v) => !v); setProfileOpen(false); }}>
            <Bell className="h-5 w-5" />
            {pending.length > 0 && <span className="absolute right-2 top-2 h-2 w-2 rounded-full bg-primary ring-2 ring-background" />}
          </Button>
          {notificationsOpen && (
            <div className="absolute right-0 top-12 w-[min(340px,calc(100vw-24px))] overflow-hidden rounded-2xl border border-border bg-background/95 shadow-2xl backdrop-blur-xl">
              <div className="flex items-center justify-between border-b border-border px-4 py-3">
                ...
                <button type="button" onClick={() => setNotificationsOpen(false)} aria-label="Close notifications" className="rounded-lg p-1.5 text-muted-foreground hover:bg-muted"><X className="h-4 w-4" /></button>
              </div>
              <div className="max-h-72 overflow-y-auto p-2">
                {pending.length ? pending.map((task) => (
                  <button key={task.id} type="button" onClick={() => { setNotificationsOpen(false); router.push('/dashboard/planner'); }} className="flex w-full items-start gap-3 rounded-xl p-3 text-left hover:bg-muted">
              ...
              <button type="button" onClick={() => { setNotificationsOpen(false); router.push('/dashboard/planner'); }} className="w-full border-t border-border px-4 py-3 text-center text-xs font-semibold text-primary hover:bg-muted">Open Planner</button>
            </div>
          )}
        </div>

        <div className="relative" ref={profileRef}>
          <Button variant="ghost" size="icon" className="h-10 w-10 rounded-xl hover:bg-muted" aria-label="Open profile" aria-expanded={profileOpen} onClick={() => { setProfileOpen((v) => !v); setNotificationsOpen(false); }}>
          ...
          {profileOpen && (
            <div className="absolute right-0 top-12 w-60 overflow-hidden rounded-2xl border border-border bg-background/95 p-2 shadow-2xl backdrop-blur-xl">
              <div className="mb-1 flex items-center gap-3 rounded-xl bg-muted/40 px-3 py-3">
              ...
              <button type="button" onClick={() => { setProfileOpen(false); router.push('/dashboard/settings'); }} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm hover:bg-muted"><UserIcon className="h-4 w-4" /> Profile</button>
              <button type="button" onClick={() => { setProfileOpen(false); router.push('/dashboard/settings'); }} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm hover:bg-muted"><Settings className="h-4 w-4" /> Settings</button>
              <button type="button" onClick={handleSignOut} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-red-500 hover:bg-red-500/10"><LogOut className="h-4 w-4" /> Sign out</button>
            </div>
          )}
        </div>
      </div>
